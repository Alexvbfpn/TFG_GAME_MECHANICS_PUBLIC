...
namespace GameMechanics.EntitiesSystem.PlayerCameras
{
    public class PlayerCameraThirdPerson : MonoBehaviour
    {
        ...
        protected string k_targetName = "Third Person Player Follower Camera Target";
        ...
        protected virtual void HandleOrbit()
        {
            if (canOrbit)
            {
                var direction = player.playerInputs.GetLookDirection();

                if (direction.sqrMagnitude > 0)
                {
                    var usingMouse = player.playerInputs.IsLookingWithMouse();
